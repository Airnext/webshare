$(document).ready(function(){
	...
	$('.btn-update').click(function(event){
		...
		$.ajax({
			...
			if(result){
				$('#profileUpdateForm').attr('action','/update/' + result._id + '?_method=PUT');
				$('#profileUpdateForm input:eq(1)').val(result.firstname);
				$('#profileUpdateForm input:eq(2)').val(result.surname);
				$('#profileUpdateForm input:eq(3)').val(result.email);
