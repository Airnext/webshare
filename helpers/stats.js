...
module.exports = function(callback){
	async.parallel([
			...
			function(next){
				ImageModel.aggregate([{$group:{_id:1, viewsTotal:{$sum:'$views'}}}], function(err, viewsTotal){
			...
			function(next){
				ImageModel.aggregate([{$group:{_id:1, likesTotal:{$sum:'$likes'}}}], function(err, likesTotal){
